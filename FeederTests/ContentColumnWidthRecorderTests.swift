...
/// observer, the launch-layout skip, the settle debounce and the write reach
/// an injected `UserDefaults` suite without a split view. Hosts a view that
/// fills an offscreen `NSHostingView`, resizes the host, and reads the suite.
...
struct ContentColumnWidthRecorderTests {
  ...
  func launchLayoutSkippedThenSettledWidthStored() async throws {
    ...
    // 450 differs from the 400 default.
    ...
    #expect(try await Self.storedValue(in: defaults, becomes: 520))

    // The sanity floor: a collapsed column is not stored.
