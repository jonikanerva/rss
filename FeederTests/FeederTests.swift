...
struct LanguageDetectionTests {
    ...
    @Test func emptyStringReturnsUnknown() {
...
struct ArticleBlockTests {
    ...
    @Test func emptyBlocksRoundtrip() {
        let blocks: [ArticleBlock] = []
        let data = blocks.toJSONData()
        #expect(data != nil)
        let decoded = [ArticleBlock].from(data!)
    ...
    @Test func classificationTextSkipsDividers() {
        let blocks: [ArticleBlock] = [
    ...
    @Test func classificationTextJoinsListItems() {
        let blocks: [ArticleBlock] = [
