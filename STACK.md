...
| A `@ModelActor` is NOT automatically off-main | `DefaultSerialModelExecutor` only *serialises* context access; `await`ed from a MainActor caller the fetch runs on **main** (issue #135). Bind a dedicated background executor (§5, §14) + assert `dispatchPrecondition(.notOnQueue(.main))` |
...
- Validate the key and taxonomy locally before any reclassification reset. A classification failure never makes Feeder select another provider. Missing Vercel credentials never select another provider. Provider, model, and key changes cancel the owned task, wait for termination, and restart with current settings.
- Every cloud request uses an ephemeral session with no cache or cookies. The request timeout is 30 seconds. The resource timeout is 60 seconds. The OpenAI request does not meet this session and timeout rule yet.
- A transient cloud failure never assigns a category. The article stays pending. Only a per-entry failure takes the uncategorized fallback, and the drain continues. OpenAI reports a context-length or content-policy rejection with HTTP 400 and an error code. The provider maps these codes to a per-entry failure.
- HTTP 408, HTTP 429, HTTP 500 to 599, a transport timeout, and a lost connection get a request retry before the drain stops. A request retry sends the same request to the same service after a short wait. The wait grows with each retry. A short valid `Retry-After` extends the retry wait. A longer valid `Retry-After` stops the drain at once without a request retry, and the loop honours the value. Other failures stop the drain without a request retry. The OpenAI request does not have the request retry yet.
...
- The providers share one retry policy when a failure has the same meaning. When the drain stops, a transport failure, HTTP 408, HTTP 429, and HTTP 500 to 599 take the loop backoff. Other HTTP failures wait for explicit Retry, a settings change, or an hourly recheck. A provider differs only when its service gives a failure a different meaning, and this section states the reason. A Vercel HTTP 402 is an exhausted budget that a top-up heals without any change in the app, so it takes the loop backoff. An OpenAI HTTP 402 keeps the hourly recheck.
