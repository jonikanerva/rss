...
- Send one HTTP attempt per article. Vercel requests use an ephemeral session with no cache or cookies. The request timeout is 30 seconds. The resource timeout is 60 seconds. The first Vercel failure stops the drain. The continuous loop owns retry delays of 30, 60, 120, and 300 seconds. Valid `Retry-After` values can extend the delay to one hour. Persisted progress resets backoff. Local configuration failures can be rechecked without HTTP. All waits are cancellable.
