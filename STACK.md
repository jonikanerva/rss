...
- Validate the key and taxonomy locally before any reclassification reset. A classification failure never makes Feeder select another provider. A missing or empty API key for the selected cloud provider never makes Feeder select another provider. Provider, model, and key changes cancel the owned task, wait for termination, and restart with current settings.
