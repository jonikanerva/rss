...
- Send one HTTP attempt per article. Every cloud request uses an ephemeral session with no cache or cookies. The request timeout is 30 seconds. The resource timeout is 60 seconds. The OpenAI request does not meet this session and timeout rule yet. The first Vercel failure stops the drain. The continuous loop owns retry delays of 30, 60, 120, and 300 seconds. Valid `Retry-After` values can extend the delay to one hour. Persisted progress resets backoff. Local configuration failures can be rechecked without HTTP. All waits are cancellable.
- Both cloud providers use the same disposition. HTTP 429 and HTTP 500 to 599 use the bounded backoff. Other HTTP failures wait for explicit Retry, a settings change, or an hourly recheck. One exception: a Vercel HTTP 402 is an exhausted budget that a top-up heals without any change in the app, so it uses the bounded backoff. An OpenAI HTTP 402 keeps the hourly recheck.
...
- A per-article rejection never stops the drain. OpenAI reports a context-length or content-policy rejection with HTTP 400 and an error code. The provider maps these codes to a per-entry failure. The entry takes the uncategorized fallback and the drain continues.
